...
module.exports = function(options) {
  ...
  // comments endpoints
  app.get("/_/comments", function(req, res) {
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify(commentsDb.getAll()));
  });

  app.post("/_/comments", function(req, res) {
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify(commentsDb.update("data", {$push: req.body})));
  });


  // Show all data for a given list
  app.get('/_/show-all/*', function(req, res) {
    var db = req.params[0];
    var dump = {};
    switch(db){
      case "lists":
  ...
  app.post("/_/list/*", function(req, res) {
    var db = req.params[0];
    res.setHeader("Content-Type", "application/json");
    var newList = listsDb.update(db, req.body);
    var additionalItems = {};
